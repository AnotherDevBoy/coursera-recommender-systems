    ...
    public AssociationModel get() {
        ...
        for (Long2ObjectMap.Entry<LongSortedSet> xEntry: itemUsers.long2ObjectEntrySet()) {
            ...
            for (Long2ObjectMap.Entry<LongSortedSet> yEntry: itemUsers.long2ObjectEntrySet()) {
                ...
                int numberOfUsersRatingX = xUsers.size();
                int numberOfUsersRatingY = yUsers.size();
                int numberOfUsers = allUsers.size();

                int numberOfUsersRatingBoth = 0;
                for (long userId: xUsers) {
                    if (yUsers.contains(userId)) {
                        ++numberOfUsersRatingBoth;
                    }
                }

                double probabilityOfXAndY = (double)numberOfUsersRatingBoth / (double)numberOfUsers;
                double probabilityOfX = (double)numberOfUsersRatingX / (double)numberOfUsers;
                double probabilityOfY = (double)numberOfUsersRatingY / (double)numberOfUsers;

                itemScores.put(yId, probabilityOfXAndY/(probabilityOfX*probabilityOfY));
            }
